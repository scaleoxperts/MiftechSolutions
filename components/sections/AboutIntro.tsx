...
export default function AboutIntro() {
  ...
  return (
    <section
      className={`relative py-12 sm:py-16 lg:py-20 overflow-hidden ${gotu.className}`}
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative order-2 lg:order-1">
            <div className="relative w-full aspect-[4/3] sm:aspect-[16/9] lg:aspect-[4/3] max-h-[480px] rounded-2xl shadow-xl overflow-hidden bg-white">
                  ...
                  <div key={index} className="min-w-full h-full relative group">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="absolute bottom-4 left-4 right-4">
                        <h3
                          className={`text-white text-xl sm:text-2xl ${cormorant.className} font-semibold`}
                        >
                          {image.title}
                        </h3>
                      </div>
              ...
              <button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-white/90 rounded-full shadow-md hover:scale-110 transition disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={isTransitioning}
                className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-white/90 rounded-full shadow-md hover:scale-110 transition disabled:opacity-50"
              >
                ...
                {images.map((_, index) => (
                  <button
                    ...
                    className={`transition-all duration-300 ${
                      index === currentIndex
                        ? "w-8 h-3 bg-[#087dc0] rounded-full"
                        : "w-3 h-3 bg-gray-300 rounded-full hover:bg-[#087dc0]"
                    }`}
                  />
                ))}
              </div>
          ...
          <div className="order-1 lg:order-2 space-y-6">
            <div className="flex items-center">
              ...
              <span
                className={`px-4 py-2 text-sm font-bold uppercase rounded-full text-[#087dc0] bg-gray-100 border border-gray-300 ${cormorant.className}`}
              >
                About MifTech
              </span>
            </div>
            ...
            <h2
              className={`text-3xl sm:text-4xl lg:text-4xl ${gotu.className} font-bold text-gray-900 leading-snug`}
            >
              Transforming Energy into{" "}
              <span
                className={`block ${cormorant.className} text-[#087dc0]`}
              >
            ...
            <p
              className={`text-lg text-gray-700 ${lato.className} leading-relaxed opacity-90`}
            >
              We build energy-efficient heat pumps that save costs, reduce
              emissions, and deliver real impact.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <a
                href="/about"
                className="px-8 py-3 text-sm font-semibold text-white bg-[#087dc0] rounded-full shadow-lg hover:scale-105 transition"
              >
                Explore Details
              </a>
              <a
                href="/products"
                className="px-8 py-3 text-sm font-semibold text-gray-900 bg-white border-2 border-gray-300 rounded-full shadow-lg hover:scale-105 hover:border-[#087dc0] hover:text-[#087dc0] transition"
              >
                More Products
