                  ...
                  {link.submenu ? (
                    <div className="relative group">
                      <button
                        ...
                        className="flex items-center px-2 lg:px-3 py-2 text-base lg:text-lg font-medium cursor-pointer transition-colors whitespace-nowrap text-[var(--dark-gray)] hover:text-[#087dc0]"
                      >
                        {link.label}
                      ...
                      <div className="absolute left-0 mt-2 w-72 bg-[var(--white)] border border-gray-200 rounded-md shadow-lg py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                        {link.submenu.map((sub) => (
                          <button
                            ...
                            className="block w-full text-left px-4 py-2 text-sm lg:text-base transition-colors text-[var(--dark-gray)] hover:text-[#087dc0] hover:bg-gray-50"
                          >
                            {sub.label}
                          </button>
                        ))}
                    ...
                    <button
                      onClick={() => navigate(link.href)}
                      className="px-2 lg:px-3 py-2 text-base lg:text-lg font-medium transition-colors whitespace-nowrap text-[var(--dark-gray)] hover:text-[#087dc0]"
                    >
                      {link.label}
                    </button>
                  )}
            ...
            {navLinks.map((link) => (
              ...
              >
                <button
                  ...
                  className="flex items-center px-3 py-2 text-lg font-medium cursor-pointer transition-colors text-[var(--dark-gray)] hover:text-[#087dc0]"
                >
                  {link.label}
                    ...
                    {link.submenu.map((sub) => (
                      ...
                      >
                        <button
                          ...
                          className="block w-full text-left px-3 py-2 text-base rounded-md transition-colors text-[var(--dark-gray)] hover:bg-[var(--linen)] hover:text-[#087dc0]"
                        >
                          {sub.label}
                        </button>
                      </motion.div>
